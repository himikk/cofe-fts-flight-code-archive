...
public :
	...
	std::string::size_type Read(float& destination, std::string::size_type offset)
	{
	...
	std::string::size_type Parse(std::string::size_type offset)
	{
		...
		while (offset != std::string::npos)
        {
            ...
            case 1 :
                offset = Read(m_Data.channel00t, ++offset) + 2;
				offset = Read(m_Data.channel00q, offset) + 2;
				offset = Read(m_Data.channel00u, offset);
                break;

            case 2 :
                offset = Read(m_Data.channel01t, ++offset) + 2;
				offset = Read(m_Data.channel01q, offset) + 2;
				offset = Read(m_Data.channel01u, offset);
                break;
			
			case 3 :
                offset = Read(m_Data.channel02t, ++offset) + 2;
				offset = Read(m_Data.channel02q, offset) + 2;
				offset = Read(m_Data.channel02u, offset);
                break;

			case 4 :
                offset = Read(m_Data.channel03t, ++offset) + 2;
				offset = Read(m_Data.channel03q, offset) + 2;
				offset = Read(m_Data.channel03u, offset);
                break;
			
			case 5 :
                offset = Read(m_Data.channel04t, ++offset) + 2;
				offset = Read(m_Data.channel04q, offset) + 2;
				offset = Read(m_Data.channel04u, offset);
                break;

			case 6 :
                offset = Read(m_Data.channel05t, ++offset) + 2;
				offset = Read(m_Data.channel05q, offset) + 2;
				offset = Read(m_Data.channel05u, offset);
                break;

			case 7 :
                offset = Read(m_Data.channel06t, ++offset) + 2;
				offset = Read(m_Data.channel06q, offset) + 2;
				offset = Read(m_Data.channel06u, offset);
                break;

			case 8 :
                offset = Read(m_Data.channel07t, ++offset) + 2;
				offset = Read(m_Data.channel07q, offset) + 2;
				offset = Read(m_Data.channel07u, offset);
                break;

			case 9 :
                offset = Read(m_Data.channel08t, ++offset) + 2;
				offset = Read(m_Data.channel08q, offset) + 2;
				offset = Read(m_Data.channel08u, offset);
                break;

			case 10 :
                offset = Read(m_Data.channel09t, ++offset) + 2;
				offset = Read(m_Data.channel09q, offset) + 2;
				offset = Read(m_Data.channel09u, offset);
                break;

			case 11 :
                offset = Read(m_Data.channel10t, ++offset) + 2;
				offset = Read(m_Data.channel10q, offset) + 2;
				offset = Read(m_Data.channel10u, offset);
                break;

			case 12 :
                offset = Read(m_Data.channel11t, ++offset) + 2;
				offset = Read(m_Data.channel11q, offset) + 2;
				offset = Read(m_Data.channel11u, offset);
                break;

			case 13 :
                offset = Read(m_Data.channel12t, ++offset) + 2;
				offset = Read(m_Data.channel12q, offset) + 2;
				offset = Read(m_Data.channel12u, offset);
                break;

			case 14 :
                offset = Read(m_Data.channel13t, ++offset) + 2;
				offset = Read(m_Data.channel13q, offset) + 2;
				offset = Read(m_Data.channel13u, offset);
                break;

			case 15 :
                offset = Read(m_Data.channel14t, ++offset) + 2;
				offset = Read(m_Data.channel14q, offset) + 2;
				offset = Read(m_Data.channel14u, offset);
                break;

			case 16 :
                offset = Read(m_Data.channel15t, ++offset) + 2;
				offset = Read(m_Data.channel15q, offset) + 2;
	...
	void InterpretBuffer()
	{
        ...
        while (start != std::string::npos)
        {
            ...
            start = Parse(++start);
			//printf("%i\n",sizeof(m_Data));
			//memcpy(&m_Data, m_buffer.c_str(),sizeof(m_Data));
